import { recursive as recursiveSelectionSort, imperative as imperativeSelectionSort } from "./selection_sort.ts";
...
function selectionSortTests(label: string, selectionSort: any) {
  Deno.test(`[${label}] it sorts an array of numbers`, function () {
    const items = Object.freeze([-100, 0, 20, 5, -3, 8000, 0, 5, 1024]) as number[];
  ...
  Deno.test(`[${label}] it handles positive and negative infinity`, function() {
    assertEquals(selectionSort([10, -Infinity, -50, Infinity]), [-Infinity, -50, 10, Infinity]);
  });
...
selectionSortTests('with recursive', recursiveSelectionSort);
selectionSortTests('with imperative', imperativeSelectionSort);
